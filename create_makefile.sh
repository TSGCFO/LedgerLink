...
# Create a properly formatted Makefile with tabs
cat > Makefile << 'EOL'
...
# Test file
TEST_FILE := tests/test_file_validator.bats
...
stalling bats-support..."; \
e https://github.com/bats-core/bats-support.git bats-test/test_helper/bats-support; \
else \
stalled."; \
fi
@if [ ! -d "bats-test/test_helper/bats-assert" ]; then \
stalling bats-assert..."; \
e https://github.com/bats-core/bats-assert.git bats-test/test_helper/bats-assert; \
else \
stalled."; \
fi
@if [ ! -d "bats-test/test_helper/bats-file" ]; then \
stalling bats-file..."; \
e https://github.com/bats-core/bats-file.git bats-test/test_helper/bats-file; \
else \
stalled."; \
fi
...
# Make sure tabs are preserved by replacing any space indentation with tabs
sed -i 's/^    /\t/g' Makefile

echo "Makefile created successfully with proper tab formatting."
